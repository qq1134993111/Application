    ...
    void GeneralApplication::Run(int argc, char* argv[])
    {
        ...
        try
        {
            ...
            AddOptionWithCallback<std::string>(
                "log-pattern",
                fmt::format("log pattern,default {}", log_config_key::default_value::kLoggerPatternValue).c_str(),
                [this](const std::string& option_name, const std::string& value)-> int32_t
                {
    ...
    bool GeneralApplication::ParseProgramOption()
    {
        try
        {
            boost::program_options::store(boost::program_options::command_line_parser(argc_, argv_)
                                          .options(options_desc_).positional(pos_desc_)
                                          .style(options_style_).run(), options_vm_);

            boost::program_options::notify(options_vm_);
        }
        catch (const boost::program_options::unknown_option& e)
        {
            std::cout << "unknown_option:" << e.what() << "\n";
            throw;
        }
        ...
        {
#if BOOST_VERSION >= 106500
            auto dump_file_path = GetOptionArgument<std::string>("display_stacktrace");
